...
import com.mo.mobileoffice.function.approval.contract.ApprovalDetailContract.Presenter;
import com.mo.mobileoffice.function.approval.presenter.ApprovalDetailPresenter;
import com.squareup.picasso.Picasso;

public class ApprovalDetailFragment extends
		MvpIdeaFragment<ApprovalDetailContract.Presenter> implements
		ApprovalDetailContract.View {
	...
	protected Presenter createPresenter() {
		return new ApprovalDetailPresenter(getActivity());
	}
	...
	private TextView appAgree;
	private TextView appRefuse;
	private TextView appCost;
	private TextView costDetail;
	private TextView costTotal;
	private GridLayout mImageViewContainer;
	...
	protected void init() {
		if (data.getApp_type().equals("1")) {
			...
			appAgree = findViewById(R.id.agree);
			appRefuse = findViewById(R.id.refuse);
			mImageViewContainer = findViewById(R.id.leave_imageContainer);
			appAgree.setOnClickListener(this);
			appRefuse.setOnClickListener(this);
	...
	public void onResume() {
		...
		if (mImageViewContainer.getChildCount() == 0) {
			...
			if (arrImages != null) {
				for (int i = 0; i < arrImages.length; i++) {
					...
					mImageViewContainer.addView(imageView);
	...
	protected int setContentViewId() {
		...
		data = (DetailData) bundle.getSerializable("detailData");
		if (data.getApp_type().equals("1")) {
			setTitle(data.getUser_name()
					+ arr[Integer.parseInt(data.getApp_type()) - 1]);
			return R.layout.layout_approval_leavedetail;
		} else if (data.getApp_type().equals("2")) {
			setTitle(data.getUser_name()
					+ arr[Integer.parseInt(data.getApp_type()) - 1]);
			return R.layout.layout_approval_resuimdetail;
		} else {
			setTitle(data.getUser_name()
					+ arr[Integer.parseInt(data.getApp_type()) - 1]);
			return R.layout.layout_approval_detail;
		}
	}
	...
	private void showAgreeDialg(final int id) {
		...
		buider.setPositiveButton("确定", new OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				dialog.dismiss();
				getPresenter().doHttpApprovalSubmit(id, data.getId(), edit);
			}
		});
